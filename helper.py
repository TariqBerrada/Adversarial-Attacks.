...
def gen_test_output(sess, logits, keep_prob, image_pl, data_folder, image_shape):
	...
	for image_file in glob(os.path.join(data_folder, 'image_2', '*.png')):
		...
		#Splicing out second column (road), reshape output back to image_shape.
		im_softmax = im_softmax[0][:, 1].reshape(image_shape[0], image_shape[1])
		#If road softmax > 0.5, then we predict a road pixel.
		segmentation = (im_softmax > 0.5).reshape(image_shape[0], image_shape[1], 1)
		#Creating mask based on segmentation to apply to original image.
...
def read_labels(labels_dir):
    """
    Reads a mask file and returns the mask corresponding to the road pixels.
    """

    image_file = scipy.misc.imread(labels_dir)
    x = np.where(image_file[:, :, 2] == 255)
    mask = np.zeros(np.shape(image_file)[:2])
    mask[x] = 1
